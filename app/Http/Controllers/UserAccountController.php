...
class UserAccountController extends Controller
{
    ...
    public function list(Request $request)
    {
        try {
            ...
            foreach ($data as $row) {
                ...
                $array[$i]["role"] = $row->userrole->role->role;
                $array[$i]['mobile_number'] = $row->mobile_number;
                $array[$i]['address'] = Str::limit($row->address, 50, '...');
    ...
    public function regenerate(Request $request)
    {
        try {
            $post = $request->all();
            $type = 'success';
            $message = "User account regenerated successfully";
            DB::beginTransaction();
            $result = UserAccount::regenerateData($post);
            if (!$result) {
                throw new Exception("Could not regenerate user account. Please try again.", 1);
            }
            DB::commit();
        } catch (QueryException $e) {
            DB::rollBack();
