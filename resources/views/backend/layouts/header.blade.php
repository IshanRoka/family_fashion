        ...
        <div class="main-header-container container-fluid" style="height: 4rem;">
            ...
            <div class="header-content-right">
                ...
                <div class="header-element headerProfile-dropdown">
                    <div class="me-3">
                        {{-- <span>Welcome! </span><a href="{{ route('admin.profile') }}"><label>@if (Auth::user()) {{ Auth::user()->name }} @endif</label></a> --}}
                    </div>
                    <!-- Start::header-link|dropdown-toggle -->
                    <div>
                        {{-- <a href="{{ route('admin.profile') }}"> --}}
                        @if (!empty(Auth::user()->image) && Storage::exists('public/profile/' . Auth::user()->image))
                            <img src="{{ asset('storage/profile/' . Auth::user()->image) }}" class="rounded-circle"
                                alt="User Profile" width=" 37" ,height="37">
                        @else
                            <img src="{{ asset('/no-user.jpg') }}" class="rounded-circle" alt="User Profile"
                                width=" 37" height="37">
                        @endif
                        </a>
                    </div>
